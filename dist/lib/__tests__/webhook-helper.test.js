...
jest.mock('../webhook-helper', () => {
  ...
  return {
    ...originalModule,
    setupWebhookServer: async (options = {}) => {
...
describe('WebhookHandler', () => {
  let webhookHandler;
  beforeEach(() => {
    webhookHandler = new webhook_helper_1.WebhookHandler({ path: '/webhook/test' });
    jest.clearAllMocks();
  });
  describe('constructor', () => {
    it('should create a webhook handler with default path', () => {
      const handler = new webhook_helper_1.WebhookHandler();
      expect(handler).toBeInstanceOf(webhook_helper_1.WebhookHandler);
      expect(handler).toBeInstanceOf(events_1.EventEmitter);
    });
    it('should create a webhook handler with custom path', () => {
      const handler = new webhook_helper_1.WebhookHandler({ path: '/custom/path' });
      expect(handler).toBeInstanceOf(webhook_helper_1.WebhookHandler);
    });
    it('should create a webhook handler with secret', () => {
      const handler = new webhook_helper_1.WebhookHandler({ secret: 'test-secret' });
      expect(handler).toBeInstanceOf(webhook_helper_1.WebhookHandler);
    });
  });
  describe('handleRequest', () => {
    it('should emit events for valid webhook payloads', () => {
      ...
      const payload = {
        type: webhook_helper_1.WebhookEventType.PROJECT_COMPLETED,
      ...
      expect(projectCompletedSpy).toHaveBeenCalledWith(
        webhook_helper_1.WebhookEventType.PROJECT_COMPLETED,
        payload
      );
    ...
    it('should check signatures when secret is set', () => {
      ...
      const secureHandler = new webhook_helper_1.WebhookHandler({ secret: 'test-secret' });
      const consoleSpy = jest.spyOn(console, 'error').mockImplementation();
      // Create a valid payload
      const payload = {
        type: webhook_helper_1.WebhookEventType.PROJECT_COMPLETED,
  ...
  describe('event handlers', () => {
    it('should register project completed handler', () => {
      ...
      webhookHandler.emit(webhook_helper_1.WebhookEventType.PROJECT_COMPLETED, {
        type: webhook_helper_1.WebhookEventType.PROJECT_COMPLETED,
        projectId: 'test',
        timestamp: new Date().toISOString(),
      });
    ...
    it('should register project failed handler', () => {
      ...
      // Manually trigger the event
      webhookHandler.emit(webhook_helper_1.WebhookEventType.PROJECT_FAILED, {
        type: webhook_helper_1.WebhookEventType.PROJECT_FAILED,
        projectId: 'test',
        timestamp: new Date().toISOString(),
      });
      expect(handler).toHaveBeenCalled();
    ...
    it('should register feedback needed handler', () => {
      ...
      // Manually trigger the event
      webhookHandler.emit(webhook_helper_1.WebhookEventType.FEEDBACK_NEEDED, {
        type: webhook_helper_1.WebhookEventType.FEEDBACK_NEEDED,
        projectId: 'test',
        timestamp: new Date().toISOString(),
      });
      expect(handler).toHaveBeenCalled();
...
describe('setupWebhookServer', () => {
  ...
  it('should set up a webhook server with default options', async () => {
    ...
    expect(webhookHandler).toBeInstanceOf(webhook_helper_1.WebhookHandler);
  ...
  it('should handle URL formatting correctly', async () => {
    ...
    const result1 = await (0, webhook_helper_1.setupWebhookServer)({
      baseUrl: 'https://example.com/',
      path: '/webhook',
    });
    expect(result1.webhookUrl).toBe('https://example.com/webhook');
    // Test without leading slash in path
    const result2 = await (0, webhook_helper_1.setupWebhookServer)({
      baseUrl: 'https://example.com',
      path: 'webhook',
    });
  ...
  it('should start the server and return a server instance', async () => {
    const { start } = await (0, webhook_helper_1.setupWebhookServer)();

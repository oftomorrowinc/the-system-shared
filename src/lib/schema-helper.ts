...
import { camelToSnake } from './case-conversion';
 ...
 */
export function convertSchemaName(name: string): string {
  if (/^[A-Z]/.test(name)) {
    // Special handling for CamelCase schema names - first letter needs to be lowercase without preceding underscore
    return name.charAt(0).toLowerCase() + camelToSnake(name.slice(1));
  }
  return name;
}
 ...
 */
export function buildSchema(schemas: Record<string, any>[]): Record<string, any> {
  ...
  for (const schema of schemas) {
    ...
    function findAndCheckCustomTypes(obj: any, path: string[] = []): void {
  ...
  const fullSchemaMap: Record<string, any> = {};
  for (const schema of schemas) {
    const schemaName = Object.keys(schema)[0];
    // Add null check
    if (schemaName) {
  ...
  function validateDataTypes(obj: any, path: string[] = []): void {
    if (Array.isArray(obj)) {
      if (obj.length === 0) {
  ...
  validateDataTypes(result[firstSchemaName], [firstSchemaName as string]);
 ...
 */
export function validateSchema(schema: Record<string, any>, data: any): any {
  ...
  function validateData(schemaValue: any, dataValue: any, path: string[]): any {
